...
from core.app_functions import remote_swipe, enable_flight_mode, disable_flight_mode, app_login_setup, app_logout_setup, \
    app_login, delete_vin, add_vin, service_reset
...
def Stolen_Vehicle_Tracking_001():
    ...
    try:
        if country == "eur":
            ...
            # manual_check(
            #     instruction="""Check for VTS license for UK based vehicles(Home screen->Settings->General->License periods)\nTap on 'My Bentley in-car services' to check the VTS
            #                     \n'My Bentley in-car services' license package should be displayed for UK based vehicle with expiry date 3+ years from now\n
            #                      Vehicle Tracking System displayed (Only EU)""",
            #     test_id="001",
            #     service=img_service,
...
def Stolen_Vehicle_Tracking_002():
    ...
    try:    
        if country == "eur":
            blocked_log("Test blocked - All done in vehicle")
            # Add block option to manual checks as this needs a non uk vehicle
            # manual_check(
            #     instruction="""Check for VTS license for non provisioned VTS vehicles(Home screen->Settings->General->License periods)\nTap on 'My Bentley in-car services' to check the VTS
            #                     \nVTS License should not be displayed for non provisioned VTS vehicles under 'My Bentley in-car services' license package""",
            #     test_id="002",
            #     service=img_service,
            #     take_screenshot=False
            # )
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if len(my_details_metrics) == 9:
                        ...
                        fail_log("Failed to extract all stolen vehicle tracking details", "006", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.click_text("My Bentley"):
                        ...
                        fail_log("My Bentley section not found", "007", img_service)

                    model = controller.d(resourceId="uk.co.bentley.mybentley:id/dropDown_vts_registration_vehicle_model").get_text()
                    metric_log(f"Vehicle Model: {model}") if model else fail_log("Model not found", "007", img_service)

                    vehicle_colour = controller.d(resourceId="uk.co.bentley.mybentley:id/dropDown_vts_registration_vehicle_color")
                    metric_log(f"Vehicle Colour: {vehicle_colour.get_text()}") if vehicle_colour else fail_log("Vehicle colour not found", "007", img_service)

                    vehicle_country = controller.d(resourceId="uk.co.bentley.mybentley:id/editText_vts_registration_vehicle_country").get_text()
                    metric_log(f"Vehicle Country: {vehicle_country}") if vehicle_country else fail_log("Vehicle country not found", "007", img_service)

                    vehicle_reg = controller.d(resourceId="uk.co.bentley.mybentley:id/editText_vts_registration_vehicle_country")
                    metric_log(f"Vehicle Registration: {vehicle_reg.get_text()}") if vehicle_reg else fail_log("Vehicle registration not found", "007", img_service)

                    if model and vehicle_colour and vehicle_country and vehicle_reg:
                        log("All details extracted from My Bentley page")
                    else:
                        fail_log("Not all details extracted from My Bentley Page", "007", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.wait_for_text(f"{current_name.split(" ")[0]}123 gqm", 30) and not controller.is_text_present("MY BENTLEY"):
                        ...
                        fail_log("Name failed to be edited", "008", img_service)
                    sleep(0.5)
                    controller.click_text("Name")
                    controller.click_by_resource_id("uk.co.bentley.mybentley:id/editText_registration_name_first_name")
                    ...
                    if not controller.wait_for_text(current_name, 30):
                        fail_log("Name failed to be reset to original name", "008", img_service)
                    ...
                    while controller.is_text_present("Loading..."):
                        sleep(1)
                    log("Email successfully edited") if controller.wait_for_text(f"newtest@gqm.anonaddy.com") else fail_log("Email failed to be edited", "008", img_service)
                    controller.click_text("Email Address")
                    controller.click_by_resource_id("uk.co.bentley.mybentley:id/editText_registration_name_email")
                    while controller.d(resourceId="uk.co.bentley.mybentley:id/editText_registration_name_email").get_text() != "Email address":
                        controller.click_by_resource_id("uk.co.bentley.mybentley:id/editText_registration_name_email")
                    ...
                    if not controller.wait_for_text(current_email, 30):
                        fail_log("Email address failed to be reset to original email", "008", img_service)
                    ...
                    log("Country can be changed") if controller.is_text_present(
                        "Search for a country or language") else fail_log("Country cannot be changed", "008",
                                                                          img_service)
                    controller.click_by_image("Icons/back_icon.png")
                    controller.click_by_resource_id(
                    ...
                    if controller.wait_for_text(f'{lines[0]}\n{lines[1]}2\n{lines[2]}\n{lines[3]}'):
                        ...
                        fail_log("Address failed to be edited", "008", img_service)
                    controller.click_text("Address")
                    controller.click_by_resource_id(
                        "uk.co.bentley.mybentley:id/editText_vts_registration_address_address")
                    ...
                    if not controller.wait_for_text(current_address.replace(".", "\n")):
                        fail_log("Address failed to be reset to original address", "008", img_service)
                    ...
                    controller.click_text("SAVE CHANGES")
                    log("Mobile number can be edited") if controller.wait_for_text("Confirm your mobile number") else fail_log("Mobile number cannot be edited", "008", img_service)
                    ...
                    if controller.is_text_present(
                        ...
                        fail_log("Correct details not displayed when security language info button clicked", "008", img_service)
                    ...
                    if controller.is_text_present("Confirm and submit"):
                        ...
                        fail_log("Security question answer failed to be edited", "008", img_service)
                    controller.click_by_image("Icons/back_icon.png")
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.wait_for_text(f"{new_colour}, {current_model}, {current_reg[:-1]}{reg_end}"):
                        ...
                        fail_log("My Bentley details not successfully edited", "009", img_service)
                    controller.click_text("My Bentley")
                    controller.click_by_resource_id("uk.co.bentley.mybentley:id/dropDown_vts_registration_vehicle_color")
                    sleep(0.5)
                    ...
                    if not controller.is_text_present(f"{current_colour}, {current_model}, {current_reg}"):
                        fail_log("My Bentley details set to original details", "009", img_service)
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.click_text("My Security Question"):
                        ...
                        if controller.is_text_present("test1"):
                            ...
                            fail_log("First security question failed to be edited", "010", img_service)
                        ...
                        if controller.is_text_present("test2"):
                            ...
                            fail_log("Second security question failed to be edited", "010", img_service)
                        ...
                        if controller.is_text_present("test3"):
                            ...
                            fail_log("Third security question failed to be edited", "010", img_service)
                        ...
                        fail_log("My security questions tab not found", "010", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.click_text("Configure"):
                        ...
                        fail_log("Configure tab not found", "011", img_service)
                    controller.swipe_up()
                    ...
                    if controller.is_text_present("Garage mode") and controller.is_text_present("Transport mode") and controller.click_text("Deactivation mode"):
                        ...
                        fail_log("Configure page not displayed successfully", "011", img_service)
                    ...
                    if controller.is_text_present("DEACTIVATION MODE ENABLED"):
                        ...
                        fail_log("Deactivation mode not displayed as enabled in 'My Alerts' page", "011", img_service)
                    if manual_run:
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if manual_run:
                        ...
                        if controller.click_text("Deactivation mode") and controller.click_text("SYNC TO CAR"):
                            ...
                            fail_log("Deactivation mode not enabled or synced to car", "012", img_service)
                    ...
                    if enable_flight_mode():
                        ...
                        fail_log("Flight mode not enabled on phone", "012", img_service)
                    sleep(1)
                    ...
                    if controller.is_text_present("An error occurred when setting special mode. Please try again."):
                        ...
                        fail_log("Expected error message not shown", "012", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if 'Currently active' in status:
                        ...
                        fail_log("Stolen vehicle tracking feature is not displayed as active", "018", img_service)
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_check("Garage mode", "019")
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_timer_check("Garage mode", "020")
                    controller.click_by_image("Icons/back_icon.png")
                    controller.swipe_down(0.05)
        else:
            blocked_log("Test blocked - Region locked (EUR)")
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_check("Transport mode", "021")
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_timer_check("Transport mode", "022")
                    controller.click_by_image("Icons/back_icon.png")
                    controller.swipe_down(0.05)
        else:
            blocked_log("Test blocked - Region locked (EUR)")
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_check("Deactivation mode", "023")
    ...
    try:
        if country == "eur":
            if app_login_setup():
                controller.click_by_image("Icons/remote_icon.png")
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    special_mode_timer_check("Deactivation mode", "024")
                    controller.click_by_image("Icons/back_icon.png")
                    controller.swipe_down(0.05)
        else:
            blocked_log("Test blocked - Region locked (EUR)")
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.is_text_present("Please disable active mode"):
                        ...
                        fail_log("Garage mode not blocked with transport mode enabled", "025", img_service)
                    controller.click_text("OK")
                    sleep(0.2)
                    controller.click_text("Transport mode")
                    controller.click_text("SYNC TO CAR")
                    ...
                    if controller.is_text_present("Sending message to car"):
                        ...
                        fail_log("Transport mode not disabled", "025", img_service)
                    while controller.is_text_present("Sending message to car"):
                        sleep(1)
                    sleep(0.2)
                    controller.click_text("STOLEN VEHICLE TRACKING")
                    ...
                    if controller.is_text_present("Sending message to car"):
                        ...
                        fail_log("Garage mode not enabled", "025", img_service)
                    while controller.is_text_present("Sending message to car"):
                        sleep(1)
                    sleep(0.2)
                    controller.click_text("STOLEN VEHICLE TRACKING")
                    ...
                    if controller.is_text_present("GARAGE MODE ENABLED"):
                        ...
                        fail_log("Garage mode not enabled successfully", "025", img_service)
                    controller.click_text("Configure")
                    sleep(0.2)
                    controller.click_text("Garage mode")
                    controller.click_text("SYNC TO CAR")
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.is_text_present("TRANSPORT MODE ENABLED"):
                        ...
                        fail_log("Transport mode not enabled", "026", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.is_text_present("NO MESSAGES"):
                        ...
                        fail_log("Special mode not deactivated", "027", img_service)
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.wait_for_text("CERTIFICATE"):
                        ...
                        fail_log("Certificate not displayed", "042", img_service)
                    sleep(1)
                    if compare_with_expected_crop("Icons/certificate_share_icon.png"):
                        log("Certificate can be downloaded")
                    else:
                        fail_log("Certificate cannot be downloaded", "042", img_service)
                    controller.click_by_image("Icons/back_icon.png")
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.wait_for_text("Call now"):
                        ...
                        fail_log("'Call now' pop up not displayed", "043", img_service)
                    sleep(1)
                    if controller.is_text_present("+44 333 122 2222"):
                        log("Vodafone contact details displayed")
                    else:
                        fail_log("Vodafone contact details not displayed", "043", img_service)
                    controller.click_text("Cancel")
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.is_text_present("STOLEN VEHICLE TRACKING"):
                        ...
                        fail_log("My Security Language changes page when clicked", "048", img_service)
                    sleep(0.2)
                    btn = controller.d.xpath('//*[@text="My Security Language"]/following-sibling::android.view.View/android.widget.Button')
                    if btn.exists:
                        controller.click(btn.info['bounds']['left']+10, btn.info['bounds']['top']+10)
                    ...
                    if controller.is_text_present("Sorry, it is not possible to change this. For further information please contact Vodafone."):
                        ...
                        fail_log("Correct details not displayed when info button clicked", "048", img_service)
                    controller.click_text("OK")
                    controller.swipe_down()
            ...
            if app_login_setup():
                ...
                if remote_swipe("STOLEN VEHICLE TRACKING"):
                    ...
                    if controller.is_text_present("Sorry, it is not possible to change this. For further information please contact Vodafone."):
                        ...
                        fail_log("Country section not unclickable like expected", "049", img_service)
                    controller.click_text("OK")
                    if not compare_with_expected_crop("Icons/Homescreen_Right_Arrow.png"):
                        log("Model edit arrow not displayed")
                    else:
                        fail_log("Model edit arrow displayed", "049", img_service)
                    controller.click_by_image("Icons/back_icon.png")
