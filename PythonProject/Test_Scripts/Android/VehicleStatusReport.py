...
def VehicleStatusReport_008():
    try:
        ...
        try:
            log(f"Fuel level: {fuel_details["fuel level"]}")
            log(f"Fuel range: {fuel_details['fuel range']}")
...
def VehicleStatusReport_009():
    try:
        ...
        try:
            log(f"Fuel level: {fuel_details["fuel level"]}")
            log(f"Fuel range: {fuel_details['fuel range']}")
            log(f"Electricity level: {fuel_details['elec level']}")
            log(f"Electricity range: {fuel_details['elec range']}")
            log(f"Combined range: {fuel_details['combined range']}")
        except Exception as e:
            error_log(e, "009")
        controller.swipe_down()
...
def VehicleStatusReport_010():
    try:
        ...
        fuel_details = controller.extract_fuel_range_and_level(phev=True)
        try:
            log(f"Total mileage: {fuel_details["total mileage"]}")
        except Exception as e:
            error_log(e, "010")
        controller.swipe_down()
    ...
    try:
        ...
        try:
            if door_details['front right'] == 'Open' and door_details['front left'] == 'Closed' and door_details['rear right'] == 'Closed' and door_details['rear left'] == 'Closed':
                ...
                log(f"Driver door: {door_details['front right']}")
    ...
    try:
        ...
        try:
            if window_details['front right'] == 'Open' and window_details['front left'] == 'Closed' and window_details[
                ...
                log(f"Driver window: {window_details['front right']}")
                log(f"Front Co Passenger window: {window_details['front left']}")
                log(f"Rear Left window: {window_details['rear left']}")
                log(f"Rear right window: {window_details['rear right']}")
                log(f"Sunroof: {window_details['sunroof']}")
        except Exception as e:
            error_log(e, "018")
    ...
    try:
        ...
        try:
            if boot_bonnet_status["boot"] == "Opened" and boot_bonnet_status["bonnet"] == "Closed":
                ...
                log(f"Boot: {boot_bonnet_status['boot']}")
                log(f"Bonnet: {boot_bonnet_status['bonnet']}")
        except Exception as e:
            error_log(e, "024")
    ...
    try:
        ...
        try:
            if light_status["lights"] == "On":
                ...
                log(f"Lights: {light_status['lights']}")
        except Exception as e:
            error_log(e, "027")
    ...
    try:
        ...
        try:
            if oil_status["oil level"] and oil_status["oil change"] and oil_status["service"]:
                ...
                log(f"Oil level: {oil_status["oil level"]}")
                log(f"Oil change: {oil_status["oil change"]}")
                log(f"Service: {oil_status['service']}")
            else:
